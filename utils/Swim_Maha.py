...
class SwimMaha:
    ...
    def mahaSampling(self, data, labels, numSamples):
        ...
        syntheticInstances = []
        data_maj_orig = data.loc[labels != self.minClass, :]
        data_min_orig = data.loc[labels == self.minClass, :]
        ...
        if (
            data_rank < T_maj.shape[0]
        ):  # there are linearly dependent column, so inverse will be singular
            if self.subSpaceSampling == False:
                ...
                QR = np.linalg.qr(data_maj_orig.values)
                indep = QR[1].diagonal() > 0
                data = data.iloc[:, indep]
                print(
                    "The majority class has linearly dependent columns. Resampled data will be in the "
                    + str(sum(indep == True))
                    + " independent columns of the orginal "
                    + str(data_maj_orig.shape[1])
                    + "-dimensional data."
                )
            ...
            try:
                ...
                if "Singular matrix" in str(e):
                    ...
                    X_new = data_min_orig[
                        np.random.choice(
                            data_min_orig.shape[0], numSamples, replace=True
                        ),
                        :,
                    ]
                    X_new = X_new + (
                        0.1 * np.random.normal(0, data_maj_orig.std(0), X_new.shape)
                    )
        ...
        try:
            M = _msqrt(
                C_inv
            )  # C_inv is the inverse of the covariance matrix, and M is the matrix for the whitening transform
            M_inv = np.linalg.inv(
                M
            )  # this is the inverse of the M matrix, we'll use it for getting the data back.
        ...
        for smpInd in range(
            smpInitPts.shape[1]
        ):  # repeat "times" times, so we get a balanced dataset
            new_w_raw = []
            new = None
            new_w = None
            ...
            for dim in range(len(min_means)):
                new_w_raw.append(
                    random.uniform(
                        smp[dim] - self.sd * min_stds[dim],
                        smp[dim] + self.sd * min_stds[dim],
                    )
                )

            ## Step 5: SCALE BACK TO THE ORIGINAL SPACE
            new_w = np.array(new_w_raw) / (
                (np.linalg.norm(new_w_raw) / np.linalg.norm(smp))
            )
        ...
        sampled_data = np.concatenate(
            [scaler.inverse_transform(np.array(syntheticInstances)), data]
        )
